/***************************************************************************
 *  include/stxxl/bits/containers/parallel_sorter_asynchron.h
 *
...
#ifndef STXXL_CONTAINERS_PARALLEL_SORTER_ASYNCHRON_HEADER
#define STXXL_CONTAINERS_PARALLEL_SORTER_ASYNCHRON_HEADER
...
#ifndef STXXL_VERBOSE_PARALLEL_SORTER_ASYNCHRON
#define STXXL_VERBOSE_PARALLEL_SORTER_ASYNCHRON STXXL_VERBOSE2
#endif
...
class parallel_sorter_asynchron : private noncopyable
{
public:
    // *** Template Parameters
...
public:
    ...
    parallel_sorter_asynchron(const cmp_type& cmp, unsigned_type memory_to_use)
        : m_state(STATE_INPUT),
    ...
    parallel_sorter_asynchron(const cmp_type& cmp, unsigned_type creator_memory_to_use, unsigned_type merger_memory_to_use)
        : m_state(STATE_INPUT),
    ...
    {
		std::cout << "parallel_sorter_asynchron" << std::endl;
        assert(m_state == STATE_INPUT);
        m_runs_creator.push(val);
    }
	
    void push_bulk(const std::vector<value_type>& val, int nthreads)
    {
        assert(m_state == STATE_INPUT);
        m_runs_creator.push_bulk(val, nthreads);
    }
    ...
    parallel_sorter_asynchron& operator ++ ()
    {
        assert(m_state == STATE_OUTPUT);
        ++m_runs_merger;
        return *this;
...
#endif // !STXXL_CONTAINERS_PARALLEL_SORTER_ASYNCHRON_HEADER
// vim: et:ts=4:sw=4
