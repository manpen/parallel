/***************************************************************************
 *  include/stxxl/bits/stream/sort_stream_parallel_asynchron.h
 *
...
private:
	...
	//queue with free blocks
	std::queue<block_type*> free_blocks;
	
	//queue with blocks to write to memory
	std::queue<std::tuple<block_type*, internal_size_type>> blocks_to_write;
	
	std::mutex mutex_free_blocks;
	
	std::condition_variable cv_free_blocks;
	
	std::mutex mutex_blocks_to_write;
...
public:
    ...
    block_type* allocate(block_type* m_blocks1)
    {
        if (!m_blocks1)
        {
	...
	void allocate()
	{
		m_blocks1 = allocate(m_blocks1);
		m_blocks2 = allocate(m_blocks2);
		clear();
	}
	...
	void deamon_write()
	{
		...
		internal_size_type m_tmp_in_run = 0;
		
		std::unique_lock<std::mutex> lk(mutex_blocks_to_write);
		...
		write_to_memory(m_blocks, m_tmp_in_run);
		
		std::unique_lock<std::mutex> lock(mutex_free_blocks);
		free_blocks.push(m_blocks);
		lock.unlock();
		}
	}
	
	
	void write_to_memory(block_type* m_blocks1, internal_size_type m_tmp_in_run)
	{
        if (m_tmp_in_run == 0)
            return;

		std::cout << "Thread: " << std::this_thread::get_id() << " writes block: " << &*m_blocks1 << " into memeory" << std::endl;
		
		// sort and store m_blocks1
        sort_run(m_blocks1, m_tmp_in_run);

        const unsigned_type cur_run_blocks = div_ceil(m_tmp_in_run, block_type::size);        // in blocks
		...
		if(m_tmp_in_run < m_el_in_run)
		{
			fill_with_max_value(m_blocks1, cur_run_blocks, m_tmp_in_run);
		}
        ...
        m_result->add_run(run, m_tmp_in_run);
	...
	block_type* get_new_block(block_type* m_blocks1, internal_size_type m_tmp_in_run)
	{
		...
		std::swap(m_blocks1, first_block);
		
		//send notification via conditional variable only if blocks to write is empty
		std::unique_lock<std::mutex> lk(mutex_blocks_to_write);
		if(blocks_to_write.empty())
		{
	    	blocks_to_write.push(std::make_tuple(first_block, m_tmp_in_run));
			cv_blocks_to_write.notify_one();
		}
		else
		{
			blocks_to_write.push(std::make_tuple(first_block, m_tmp_in_run));
	...
	void parallel_push(vector_iterator first, vector_iterator last)
	{
	    //! current number of elements in the run m_blocks1
		...
		for(; it < last; it++)
		{
			...
			{
				m_blocks3[m_cur_el / block_type::size][m_cur_el % block_type::size] = *it;
				++m_cur_el;
	        }
			else
			{
				assert(m_el_in_run == m_cur_el);
				m_blocks3 = get_new_block(m_blocks3, m_el_in_run);
				m_cur_el = 0;
				m_blocks3[m_cur_el / block_type::size][m_cur_el % block_type::size] = *it;
				std::cout << "After swap Pointer m_block3: " << &m_blocks3 << " Memory block: " << &*m_blocks3 << std::endl;
			}
		}
			//add the remaining blocks which are not fully filled to the queue
	        get_new_block(m_blocks3, m_cur_el);
		
	}
	
	//Takes an vector of values to be sorted
	void push_bulk(const std::vector<value_type>& val, unsigned int nthreads)
	{
		push_bulk_used = true;
		done_pushing = false;
		unsigned long const length = val.size();
		...
		//currently not implemented to better test behaviour with different number of threads
		//unsigned long const min_per_thread = block_type::size;
		...
		for(unsigned long i=0; i<(nthreads-1);i++)
		{
			...
			vector_iterator chunk_end = chunk_start;
			std::advance(chunk_end, chunk_size);
			threads[i] = std::thread(&runs_creator::parallel_push, this, chunk_start, chunk_end);
		...
		//std::thread t(&runs_creator::deamon_write, this);
		done_pushing = true;
		// send last notify in case deamon_write is already waiting due to empty write_to_blocks queue
        std::unique_lock<std::mutex> lk(mutex_blocks_to_write);
		cv_blocks_to_write.notify_one();
		lk.unlock();
		...
		//final check if all write requests are finished
		finished_writing();
		
		//deallocate all blocks in free_blocks queue
